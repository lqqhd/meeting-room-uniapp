<template>
  ...
  <view class="container">
    <view class="grid">
      ...
      <i-input id="name" :value="entry.name" title="姓名" placeholder="填写姓名，便于与同事沟通" maxlength="50" @change="bindChange" right :errorMessage="errMsgs.nameErrMsg"></i-input>
      ...
      <i-input id="valicode" :value="entry.password" title="验证码" placeholder="请填写手机验证码" maxlength="200" @change="bindChange" right :errorMessage="errMsgs.passwordErrMsg"></i-input>
      <view class="btn-box">        
  ...
  methods: {
    ...
    async getDict() {
      for (let fieldId in this.dictTypes) {
        let dictType = this.dictTypes[fieldId]; //先从缓存中获取
