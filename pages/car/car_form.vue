<template>
  ...
  <view class="container">
    <view class="grid">
      ...
      <i-input id="xh" :value="entry.xh" type="number" title="序号" placeholder="请填写序号" maxlength="11" @change="bindChange" right :errorMessage="errMsgs.xhErrMsg"></i-input>
      <i-input id="num" :value="entry.num" title="车牌号" placeholder="请填写车牌号" maxlength="64" @change="bindChange" right :errorMessage="errMsgs.numErrMsg"></i-input>
      <i-input id="name" :value="entry.name" title="车辆名称" placeholder="请填写车辆名称" maxlength="64" @change="bindChange" right :errorMessage="errMsgs.nameErrMsg"></i-input>
      <picker id="seatNum" @change="bindChange" :value="entry.seatNum" :range="seatNum_array">
        <i-input :value="seatNum_array[entry.seatNum] + '人'" title="准乘人数" placeholder="请选择准乘人数" disabled="true" right rightIcon="enter" rightIconSize="18"></i-input>
      </picker>
      <i-cell title="状态">
        <i-switch id="status" :value="entry.status" @change="bindSwitchChange" slot="footer"></i-switch>
      </i-cell>
      <picker id="nextCheckDate" mode="date" start="2019-01-01" @change="bindChange">
        <i-input :value="entry.nextCheckDate" title="下一次年检日期" placeholder="请选择下一次年检日期" maxlength="0" @change="bindChange" disabled="true" right rightIcon="activity" rightIconColor="#0095F2" :errorMessage="errMsgs.nextCheckDateErrMsg"></i-input>
      </picker>
    </view>
    <view class="btn-box">
